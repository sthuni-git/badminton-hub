  ...
  return <main className="min-h-screen pb-24">
    ...
    <Sheet open={!!selected} onOpenChange={open=>!open&&setSelected(null)}>
      {selected && <SheetContent side="bottom" className="mx-auto max-h-[90vh] max-w-2xl overflow-y-auto rounded-t-[28px] border-x px-0 pb-5 sm:bottom-6 sm:rounded-[28px] sm:border">
        ...
        <div className="grid grid-cols-2 gap-2 px-5 pt-4 sm:px-7"><Button variant="outline" className="h-11 rounded-xl" render={<a href={`https://map.naver.com/p/search/${encodeURIComponent(selected.venue)}`} target="_blank" rel="noreferrer"/>}><MapPin/> 길찾기</Button><Button variant="outline" className="h-11 rounded-xl" render={<a href={googleCalendarUrl(selected)} target="_blank" rel="noreferrer"/>}><CalendarDays/> 캘린더 추가</Button><Button className="col-span-2 h-12 rounded-xl bg-emerald-700 text-base hover:bg-emerald-800" render={<a href={selected.officialLink} target="_blank" rel="noreferrer"/>}>접수·요강 보러가기 <ExternalLink/></Button></div>
      </SheetContent>}
    </Sheet>
  </main>;
}
...
function TournamentCard({ tournament:t, onSelect }: { tournament:Tournament; onSelect:()=>void }) {
  ...
  return <article className="group rounded-2xl border bg-white p-4 shadow-[0_4px_20px_rgb(20_83_45/5%)] transition hover:-translate-y-0.5 hover:border-emerald-200 hover:shadow-[0_10px_32px_rgb(20_83_45/10%)] sm:p-5">
    ...
    <div className="mt-4 flex items-center justify-between border-t pt-3"><div><span className="text-xs font-bold text-foreground">{t.source}</span><span className="ml-2 text-xs text-muted-foreground">{status==='마감'?'접수 마감':regD===0?'오늘 마감':regD>0?`접수 D-${regD}`:'접수 종료'}</span></div><Button size="sm" className="rounded-lg bg-emerald-700" render={<a href={t.officialLink} target="_blank" rel="noreferrer"/>}>요강 보기 <ChevronRight/></Button></div>
  </article>;
}
